export const CORE_SHADOWS = {
  'shadow-1': `inset 0 0 0 1px var(--grey-a5),
     inset 0 1.5px 2px 0 var(--grey-a2),
     inset 0 1.5px 2px 0 var(--black-a2)`,

  'shadow-2': `0 0 0 1px var(--grey-a3),
     0 0 0 0.5px var(--black-a1),
     0 1px 1px 0 var(--grey-a4),
     0 2px 1px -1px var(--black-a1), 0 1px 3px 0 var(--black-a1)`,

  'shadow-3': `0 0 0 1px var(--grey-a3),
     0 2px 3px -2px var(--grey-a3),
     0 3px 12px -4px var(--black-a2),
     0 4px 16px -8px var(--black-a2)`,

  'shadow-4': `0 0 0 1px var(--grey-a3),
     0 8px 40px var(--black-a1),
     0 12px 32px -16px var(--grey-a3)`,

  'shadow-5': `0 0 0 1px var(--grey-a3),
     0 12px 60px var(--black-a3),
     0 12px 32px -16px var(--grey-a5)`,

  'shadow-6': `0 0 0 1px var(--grey-a3),
     0 12px 60px var(--black-a3),
     0 16px 64px var(--gray-a2),
     0 16px 36px -20px var(--gray-a7)`,
}
